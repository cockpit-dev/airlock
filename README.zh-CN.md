...
- **控制台** — SvelteKit 5 管理界面，包含 token 登录、可选 Google OAuth、
  指标、密钥管理、Providers、Routes、Accounts、路由健康和 OpenAI Responses
  Playground。
...
pnpm build         # 构建所有包和应用
pnpm e2e           # Dashboard Playwright E2E 测试
...
### Dashboard 可选 OAuth 变量

只有当以下三个变量都配置完整时，Dashboard 登录页才会显示 Google OAuth 入口：

| 变量                   | 说明                                   |
| ---------------------- | -------------------------------------- |
| `AUTH_SECRET`          | Auth.js session secret（至少 32 字符） |
...
pnpm build          # 构建所有包
pnpm e2e            # 运行 Dashboard Playwright E2E 测试
pnpm run audit      # 生产依赖安全审计
```

当前验证套件包含 2000+ 个 Vitest 单元/集成测试，并有 Dashboard Playwright E2E
覆盖。覆盖范围包括协议、canonical 管线、provider、路由、治理、请求整形、
遥测、网关集成和 Dashboard 工作流。
