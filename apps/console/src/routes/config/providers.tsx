...
function ProvidersPage() {
  ...
  function handleSave() {
    ...
    putSection.mutate(updated, {
      onSuccess: () => {
        toast.success(
          `Provider "${editId}" ${isEditing ? "updated" : "created"} successfully.`
        );
        modalState.close();
      },
      onError: (err) => {
        toast.danger(
          `Failed to save provider: ${err instanceof Error ? err.message : "Unknown error"}`
        );
      }
    });
  }
  ...
  function handleDelete() {
    ...
    putSection.mutate(updated, {
      ...
      onError: (err) => {
        toast.danger(
          `Failed to delete provider: ${err instanceof Error ? err.message : "Unknown error"}`
        );
      }
    });
  }
      ...
      {section.isLoading ? (
        ...
        <Card.Root>
          <Card.Header className="flex-row items-center justify-between">
            ...
            <span className="text-xs text-muted">{providers.length} total</span>
          </Card.Header>
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        >
                          <FiRefreshCw
                            size={12}
                            className={
                              fetchModels.isPending ? "animate-spin" : ""
                            }
                          />
                          Models
                        </Button>
          ...
          <Modal.Dialog>
            ...
            <Modal.Body className="gap-3">
              <TextField value={editId} onChange={setEditId}>
                ...
                <Input placeholder="e.g. openai-primary" readOnly={isEditing} />
                {isEditing && (
                  <Description>
