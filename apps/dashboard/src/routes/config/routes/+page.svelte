<script lang="ts">
  ...
  const currentProviderModels = $derived(
    providerModelsMap[formTargetProvider] ?? []
  );
  ...
  function loadFormFromRoute(r: RouteConfig) {
    formExternalModel = r.externalModel;
    formTargetProvider = r.target.provider;
    formTargetModel = r.target.providerModel;
    formFallbacks = r.fallbacks ? r.fallbacks.map((f) => ({ ...f })) : [];
    formStrategy = r.strategy ?? "";
    formRequiredKeyTier = r.requiredKeyTier ?? "";
    formRequiredKeyTags = r.requiredKeyTags?.join(", ") ?? "";
  }
  async function loadConfig() {
    ...
    try {
      ...
      if (sec?.data && Array.isArray(sec.data))
        routes = sec.data as RoutesConfig;
  ...
  async function saveRoutes() {
    ...
    success = "";
    try {
      await cl.putConfigStoreSection("routes", routes);
      success = "Saved";
      editRouteIndex = null;
  ...
  function buildRoute(): RouteConfig {
    return {
  ...
  function applyEdit() {
    ...
    error = "";
    routes = routes.map((r, i) => (i === editRouteIndex ? buildRoute() : r));
    editRouteIndex = null;
    resetForm();
    saveRoutes();
  }
  function deleteRoute(i: number) {
  ...
  function removeFallback(i: number) {
    formFallbacks = formFallbacks.filter((_, j) => j !== i);
  }
  function updateFallback(
    i: number,
    f: "provider" | "providerModel",
    v: string
  ) {
    formFallbacks = formFallbacks.map((fb, j) =>
      j === i ? { ...fb, [f]: v } : fb
    );
  }
  loadConfig();
...
<div class="flex items-center justify-between mb-3 mt-1">
  ...
  {#if !loading && providerKeys.length > 0}
    <Button size="sm" onclick={startCreate}
      ><Plus data-icon="inline-start" />Add Route</Button
    >
  {/if}
</div>
...
{:else if providerKeys.length === 0}
  <Card.Root>
    <Card.Content>
      ...
      <Button variant="outline" size="sm" href="/config/providers" class="mt-2"
        >Add Provider</Button
      >
    </Card.Content>
  </Card.Root>
  ...
  <div class="grid gap-1.5">
    {#each routes as route, index}
      ...
      {:else}
        <Card.Root size="sm">
          <Card.Content
            class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5"
          >
    ...
    <Card.Content class="grid gap-3">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-2.5">
        ...
        <div class="grid gap-1">
          ...
          >
            <option value="">Default</option>{#each strategyOptions as s}<option
                value={s}>{s}</option
              >{/each}
          </select>
        </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-1.5">
          ...
          <Button variant="ghost" size="xs" onclick={addFallback}
