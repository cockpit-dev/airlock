          ...
          <Card.Content>
            <div class="flex items-center justify-between mb-2">
              <div class="flex items-center gap-2">
                <span
                  class="inline-block size-2 rounded-full {provider.configured
                    ? 'bg-success'
                    : 'bg-muted-foreground'}"
                ></span>
            ...
            <div class="flex flex-col gap-1 text-xs">
              <div class="flex justify-between">
                <span class="text-muted-foreground">Adapter</span><span
                  >{providerLabels[provider.type] ?? provider.type}</span
                >
              </div>
              <div class="flex justify-between">
                <span class="text-muted-foreground">Base URL</span>
                <span class="font-mono text-[11px] truncate ml-2"
        ...
        {#each data.config.routes as route}
          <Table.Row>
            <Table.Cell class="font-mono text-xs font-medium"
              >{route.externalModel}</Table.Cell
            >
            <Table.Cell
            ...
            >
            <Table.Cell
              >{#if route.fallbacks?.length}{#each route.fallbacks as fb}<span
                    class="text-[11px]"
                    ><span class="text-muted-foreground">{fb.provider}</span
                    >/<span class="font-mono">{fb.providerModel}</span></span
                  >{/each}{:else}<span class="text-muted-foreground text-xs"
                  >-</span
                >{/if}</Table.Cell
            >
          ...
          <Card.Root size="sm">
            <Card.Content>
              ...
              <div class="flex flex-wrap gap-1">
                {#each models as model}<Badge variant="outline">{model}</Badge
                  >{/each}
              </div>
            </Card.Content>
          </Card.Root>
...
{:else if !hasCreds}
  <Card.Root class="border-warning/30 bg-warning/5">
    <Card.Content>
      <p class="text-sm">
        Connect to a gateway admin endpoint to view configuration.
      </p>
