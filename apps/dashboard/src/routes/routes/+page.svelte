        ...
        <div class="bg-gray-900 border border-gray-800 rounded-lg p-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p class="text-white">
                {(
                  data.routingHealth.config.circuitBreakerPolicy.cooldownMs /
                  1000
                ).toFixed(0)}s
              </p>
            </div>
            ...
            <tbody>
              {#each Object.entries(data.routingHealth.targets) as [name, target]}
                ...
                <tr class="border-b border-gray-800 last:border-0">
                  ...
                  <td class="px-4 py-3">
                    <span
                      class="px-2 py-1 rounded text-xs font-medium {stateColor(
                        state
                      )}"
                    >
                      {state}
                  ...
                  <td class="px-4 py-3">
                    {#if target.metrics?.recoveryScore != null}
                      <span class="text-gray-300"
                        >{(target.metrics.recoveryScore * 100).toFixed(
                          0
                        )}%</span
                      >
                    {:else}
                  ...
                  <td class="px-4 py-3 text-gray-400 text-xs">
                    {#if target.metrics?.freshness}
                      <span
                        >lat: {target.metrics.freshness.latencyFreshMs ??
                          "-"}ms</span
                      >
                      <span class="mx-1">|</span>
                      <span
                        >fail: {target.metrics.freshness.failureFreshMs ??
                          "-"}ms</span
                      >
                    {:else}
                      -
