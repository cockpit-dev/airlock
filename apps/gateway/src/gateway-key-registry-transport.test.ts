...
describe("isGatewayKeyRegistryEnabled", () => {
  it("is disabled without a registry Durable Object binding", () => {
    expect(isGatewayKeyRegistryEnabled(createEnv())).toBe(false);
  });

  it("enables the registry when the Durable Object binding exists", () => {
    expect(
      isGatewayKeyRegistryEnabled(
        createEnv({
          AIRLOCK_GATEWAY_KEY_REGISTRY: {
            idFromName: () => "registry",
            get: () => ({
