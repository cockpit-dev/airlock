...
describe("gateway app", () => {
  ...
  it("preserves chat assistant text when gemini returns mixed text and functionCall", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          responseId: "gemini-response-123",
  ...
  it("preserves chat text before tool deltas when gemini streams mixed text and functionCall", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        new ReadableStream<Uint8Array>({
          start(controller) {
            controller.enqueue(
              encoder.encode(
                [
                  'data: {"responseId":"gemini-response-123","modelVersion":"gemini-2.5-flash","candidates":[{"content":{"role":"model","parts":[{"text":"Let me check that."},{"functionCall":{"name":"lookup_weather","args":{"city":"Shanghai"}}}]}}]}\n\n',
  ...
  it("accepts chat service_tier=scale and store=null for OpenAI compatibility", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          id: "chatcmpl_123",
  ...
  it("preserves native openai chat logprobs in streaming events when explicitly requested", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        new ReadableStream<Uint8Array>({
    ...
    expect(body).toContain(
      '"choices":[{"index":0,"delta":{"content":"hello"},"logprobs":{"content":[{"token":"hello","logprob":-0.1,"top_logprobs":[{"token":"hello","logprob":-0.1}]}]},"finish_reason":null}]'
    );
    expect(body).toContain(
      '"choices":[{"index":0,"delta":{},"logprobs":{"content":[{"token":"hello","logprob":-0.1,"top_logprobs":[{"token":"hello","logprob":-0.1}]}]},"finish_reason":"stop"}]'
    );
  });
  ...
  it("fails closed when chat logprobs controls are sent to Gemini", async () => {
    const fetcher = vi.fn();
    const app = createApp({ fetcher });
  ...
  it("treats stale closed-target retryable failure counts as neutral for priority routing in the app", async () => {
    const currentTime = Date.now();
    const routeFetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          id: "chatcmpl_app_priority_stale_failure_count",
          object: "chat.completion",
          created: 1,
          model: "gpt-4.1-mini",
          choices: [
  ...
  it("accepts responses service_tier=scale for OpenAI compatibility", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          id: "resp_123",
  ...
  it("fails closed when responses output-text logprobs controls are sent to Gemini", async () => {
    const fetcher = vi.fn();
    const app = createApp({ fetcher });
  ...
  it("fails closed when responses text.verbosity is sent to Anthropic", async () => {
    const app = createApp({ fetcher: vi.fn() });
  ...
  it("fails closed when responses text.verbosity is sent to gemini", async () => {
    const app = createApp({ fetcher: vi.fn() });
    ...
    await expect(readJson(response)).resolves.toEqual({
      error: {
        message:
          "Provider gemini does not support required capability: openai_responses_text_controls",
        type: "routing",
        code: "provider_capability_not_supported"
      }
    });
  ...
  it("fails closed when responses truncation is sent to Anthropic", async () => {
    const app = createApp({ fetcher: vi.fn() });
  ...
  it("fails closed when responses truncation is sent to gemini", async () => {
    const app = createApp({ fetcher: vi.fn() });
  ...
  it("preserves responses output_text when gemini returns mixed text and functionCall", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          responseId: "gemini-response-123",
  ...
  it("preserves responses text output before tool items when gemini streams mixed text and functionCall", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        new ReadableStream<Uint8Array>({
  ...
  it("does not inject responses stream_options include_obfuscation=false unless explicitly requested", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        new ReadableStream<Uint8Array>({
          start(controller) {
            controller.enqueue(
              encoder.encode(
                [
                  'data: {"type":"response.created","sequence_number":0,"response":{"id":"resp_123","object":"response","created_at":1,"model":"gpt-4.1-mini","status":"in_progress","output":[],"tools":[]}}\n\n',
                  'data: {"type":"response.completed","sequence_number":1,"response":{"id":"resp_123","object":"response","created_at":1,"model":"gpt-4.1-mini","status":"completed","output":[],"tools":[]}}\n\n',
                  "data: [DONE]\n\n"
                ].join("")
              )
            );
  ...
  it("preserves messages text blocks before tool_use when gemini streams mixed text and functionCall", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        new ReadableStream<Uint8Array>({
  ...
  it("preserves messages text blocks when gemini buffered response contains mixed text and functionCall", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          responseId: "gemini-response-123",
  ...
  it("preserves mixed text and tool_use blocks when messages ingress routes to openai responses", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          id: "resp_123",
  ...
  it("streams tool_use argument deltas when messages ingress routes to openai responses", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        new ReadableStream<Uint8Array>({
            ...
            controller.enqueue(
              encoder.encode(
                [
                  'data: {"type":"response.created","sequence_number":0,"response":{"id":"resp_123","object":"response","created_at":1,"model":"gpt-4.1-mini","status":"in_progress","output":[],"parallel_tool_calls":true,"tools":[]}}\n\n',
                  'data: {"type":"response.output_item.added","sequence_number":1,"output_index":0,"item":{"type":"function_call","call_id":"call_123","name":"lookup_weather","arguments":"","status":"in_progress"}}\n\n',
                  'data: {"type":"response.function_call_arguments.delta","sequence_number":2,"item_id":"call_123","output_index":0,"delta":"{\\"city\\":\\"Shanghai\\"}"}\n\n',
                  'data: {"type":"response.completed","sequence_number":3,"response":{"id":"resp_123","object":"response","created_at":1,"model":"gpt-4.1-mini","status":"completed","output":[{"type":"function_call","call_id":"call_123","name":"lookup_weather","arguments":"{\\"city\\":\\"Shanghai\\"}","status":"completed"}],"parallel_tool_calls":true,"tools":[],"usage":{"input_tokens":11,"output_tokens":5,"total_tokens":16}}}\n\n',
                  "data: [DONE]\n\n"
                ].join("")
              )
            );
  ...
  it("preserves text and tool block ordering when messages ingress routes to openai responses streaming", async () => {
    const encoder = new TextEncoder();
    const fetcher = vi.fn().mockResolvedValueOnce(
      new Response(
        new ReadableStream<Uint8Array>({
