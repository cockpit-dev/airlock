...
describe("AnthropicProviderAdapter", () => {
  ...
  it("forwards canonical required tool choice to Anthropic as any", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          id: "msg_123",
  ...
  it("forwards canonical none tool choice to Anthropic", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          id: "msg_123",
  ...
  it("parses upstream streamed anthropic tool_use starts without input_json_delta", async () => {
    const encoder = new TextEncoder();
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(
